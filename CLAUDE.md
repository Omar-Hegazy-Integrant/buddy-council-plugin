...
- **MCP Servers** (`mcp-servers/`) — standalone MCP servers wrapping external APIs (TestRail, Jama). GitHub access is handled either via the official external `github-mcp-server` (not vendored) or via the `gh` CLI. Jira/Confluence use the **`sooperset/mcp-atlassian` server**, launched with `uvx` at a pinned version (`mcp-atlassian@0.23.1`). It supports **both Jira Cloud and Server/Data Center** — which the retired official server did not, being Cloud-only — also not vendored: `/bc:setup` writes it into both runtimes' MCP configs, like TestRail. It runs through the `uv` the plugin already requires, so there is no Docker dependency and no daemon. Never vendor a Jira server again.
...
- No hardcoded secrets — credentials live only under `~/.buddy-council/` (gitignored, `chmod 600`), in two files because the servers read two formats: `secrets.json` for the ones that read JSON (TestRail, GitHub), and `atlassian.env` for the Atlassian server, which takes environment variables via `--env-file` and cannot read JSON. `.mcp.json` is gitignored and still holds no secrets — only non-secret env (`*_BASE_URL`), `BC_SECRETS_FILE`, and the `--env-file` *path*. The external GitHub MCP server is the lone exception (its token stays in `.mcp.json` env). **Jira now requires a credential** — that inverted in 0.20.0 when the OAuth server was dropped; the API token belongs in `atlassian.env`, never in `sources.json` or an MCP config.
...
- **The `atlassian` server is no longer declared in any plugin manifest.** Its `--env-file` path is absolute and per-machine, so it cannot be committed — `/bc:setup` writes the entry into `.mcp.json` (Claude Code) and `~/.copilot/mcp-config.json` (Copilot CLI), exactly like TestRail. This also sidesteps Copilot not merging plugin-declared MCP servers ([#2709](https://github.com/github/copilot-cli/issues/2709)). **Pin the version** in both configs: the server changed its default `TOOLSETS` in 0.22.0, and floating to latest would silently change which tools exist.
...
- **`jira`** — `{base_url, deployment, project_key, default_issue_type, board: {url, id}, pending}`. **`deployment`** is `"cloud"` or `"server"`, detected from `/rest/api/2/serverInfo` and recorded so a later migration is noticed. It selects both the auth shape (Cloud: `JIRA_USERNAME`+`JIRA_API_TOKEN`; Server/DC: `JIRA_PERSONAL_TOKEN`) and the REST API version used by setup's verification calls (Cloud `/rest/api/3`, Server/DC `/rest/api/2` — v3 does not exist there). The Agile API is unversioned and identical on both. Never write both auth shapes into the env file: the server picks its method from which variables are present, so a leftover pair after a Cloud migration yields 401s that look like a bad token. **Written on every `/bc:setup` run — Step 3 is required, not optional.** `board.id` is the integer identifying the dev board and is passed straight to `jira_get_board_issues`; `project_key` comes from the board and wins over any separately chosen project. `pending: true` means the board was recorded but never verified because the server or the credentials were not ready at setup time — `/bc:setup` re-verifies and clears it on the next run, analysis commands skip the board source while it is set, and `/bc:validate` refuses to create real tickets. There is deliberately **no credential here**: it lives in `~/.buddy-council/atlassian.env`. A `cloud_id` in an existing config is a pre-0.20.0 leftover — the server is bound to one site by `JIRA_URL`, so no tool takes a site identifier; drop it.
- **`jira.vnv_board`** — `{url, id, project_key}`. The V&V team's board, used by `/bc:vnv-sprint-prep`. Optional in `/bc:setup` (unlike the dev board) because `/bc:vnv-sprint-prep` can collect it itself. **Must be a different *board* from `jira.board.id`** — setup and the agent refuse only a same-board value, because cloning onto the source board is always wrong. A shared **project** is legitimate: one Jira project can host many boards, each with its own filter. When `vnv_board.project_key == jira.project_key`, clones live in the dev project, so `fetch-board-issues` excludes issues carrying a `jira.vnv_workflow.labels` pipeline label — otherwise `/bc:coverage` would double-count each clone against the story it copies.
