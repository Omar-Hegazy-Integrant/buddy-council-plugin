...
- **MCP Servers** (`mcp-servers/`) — standalone MCP servers wrapping external APIs (TestRail, Jama). GitHub access is handled either via the official external `github-mcp-server` (not vendored) or via the `gh` CLI. Jira/Confluence use Atlassian's **official hosted remote server** (`https://mcp.atlassian.com/v1/mcp/authv2`, browser OAuth) — also not vendored: it is declared in `.claude-plugin/plugin.json` for Claude Code and written to `~/.copilot/mcp-config.json` by `/bc:setup` for Copilot CLI. Never vendor a Jira server again.
...
- No hardcoded secrets — credentials live only in `~/.buddy-council/secrets.json` (gitignored, `chmod 600`), the single source of truth. `.mcp.json` is gitignored and holds no secrets — only non-secret env (`*_BASE_URL`) plus `BC_SECRETS_FILE` (the path the MCP servers read). The external GitHub MCP server is the lone exception (its token stays in `.mcp.json` env). Jira needs no credential at all — Atlassian's server authenticates with browser OAuth and holds its own grant, so never add a `jira` section to the secrets file.
- **The `atlassian` server is registered once per runtime, never twice**: `.claude-plugin/plugin.json` → `mcpServers.atlassian` for Claude Code (live at install time), `~/.copilot/mcp-config.json` for Copilot CLI (written by `/bc:setup`). It is deliberately absent from `.mcp.json` and from `.plugin/plugin.json`: Copilot doesn't merge plugin-declared MCP servers into its runtime config ([#2709](https://github.com/github/copilot-cli/issues/2709)) and never prompts OAuth for plugin-sourced HTTP servers ([#1967](https://github.com/github/copilot-cli/issues/1967)), so a manifest entry there would shadow the working one with a silently unauthorized duplicate.
