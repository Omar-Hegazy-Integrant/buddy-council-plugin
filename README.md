...
- Jira access **does** hold a stored credential as of 0.20.0: an Atlassian API token in `~/.buddy-council/atlassian.env` (`chmod 600`), read by the Atlassian server via `--env-file`. It never appears in `sources.json` or in either MCP config — those reference the file by path
