...
- When creating Jira tickets, use the `jira_create_issue` tool from the `mcp-atlassian` server (`mcp__atlassian__jira_create_issue` under Claude Code, `jira_create_issue` under Copilot CLI).
...
- **`jira.pending` is `true`** → the board was recorded but the connection test never passed. Without `--dry-run`, stop: "Your Jira board is recorded but the connection was never verified. Re-run `/bc:setup` to re-test it — if Jira is self-hosted, check you're on the VPN first. `--dry-run` works in the meantime."
...
There is no `cloud_id` and no site resolution step — `mcp-atlassian` is bound to a single site by `JIRA_URL`
in `~/.buddy-council/atlassian.env`. If a `cloud_id` survives in the config from before 0.21.1, ignore it.
...
- `issue_type`: from `jira.default_issue_type` (or default to "Story")
- `summary`: from draft
- `description`: from draft, as markdown. The server converts it — to ADF on Cloud, to wiki markup on
  Server/DC — so pass markdown and do not pre-convert.
- `additional_fields`: any extra fields, e.g. `{"labels": [...]}`. Omit when there are none.

**Then place it in the board's active sprint**, so it lands on the board rather than in the backlog. Follow
the *Sprints* section of `${CLAUDE_PLUGIN_ROOT}/skills/fetch-board-issues/SKILL.md`:
`jira_get_sprints_from_board` (`board_id: jira.board.id`, `state: "active"`) → `jira_add_issues_to_sprint`
with the returned `sprint_id` and the new issue key.

This is a **post-create** step, not a create field: `jira_add_issues_to_sprint` is the supported path, and
guessing a `customfield_*` sprint id on create is not. If there is no open sprint, or the board is Kanban,
or the add fails, keep the ticket and say it landed in the backlog — never fail creation over sprint
placement.
...
- **401 / not authorized** → the token in `~/.buddy-council/atlassian.env` is wrong or expired. Tell the user
  to re-run `/bc:setup` to re-enter it. There is no browser re-authorization step.
- **403 / permission denied** → the account lacks "Create Issues" on that project, or an IP allowlist is
  blocking the request. Say which of the two you cannot distinguish.
...
- **About the created ticket** (e.g., "can you update the description?", "add another acceptance criterion"):
  ...
  - The MCP server does expose `jira_update_issue`, but this agent's workflow does not cover re-validating
    an edit against requirements, so it is deliberately not wired up.
...
- If MCP tools are not available → tell the user to check `.mcp.json` (both `testrail` and `atlassian`) and restart the runtime. There is no authorization step for Atlassian; if the server is listed but every call fails, the token in `~/.buddy-council/atlassian.env` is wrong or the host is unreachable without a VPN.
